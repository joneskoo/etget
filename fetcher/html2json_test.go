...
func TestDateConverterPositive(t *testing.T) {
	...
	for _, c := range cases {
		...
		if c.error && err == nil {
			t.Errorf("dateConverter(%q); expected error, got err='%v'", c.in, err)
		}
		if !c.error && err != nil {
			t.Errorf("dateConverter(%q); expected err=nil, got err='%v'", c.in, err)
		}
		if out != c.want {
			t.Errorf("dateConverter(%q); expected '%v', got '%v'", c.in, c.want, out)
		}
	}
}
